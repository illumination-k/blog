...
という反応を起こす逆転写酵素(Reverse Transcriptase)を持ちます。この酵素を用いることで、RNAからcDNA (RNA由来のDNAをcDNAと呼ぶ) を合成することができます。DNAに変換したあとにPCR反応を行うことができるようになり、極微量のRNAを検出することが可能になります。

RNA-seqのときの最初のステップとして行われる反応です。この反応にもプライマーが必要で、プライマーの種類によって増幅され方が違います。これらのプライマーにさらに何らかの配列（例えばバーコードやアダプター）を付加することで、それらの配列が付加されたcDNAを合成することができます。

### 1. polyA配列を付加したオリゴdTプライマー

真核生物のmRNAの3'末端には連続したA (polyA) 配列を持ちます。なので、連続したT (polyT) 配列を持つプライマーを用いてcDNAを増幅することでmRNAのみを狙って増幅することが可能です。ただ、この方法では原核細胞やmiRNAなどには使用することができません。RNA-seqの場合は基本的にこのプライマーを使うことが多いと思います。
...
狙った遺伝子のcDNAのみを合成することができます。基本的にはRT-PCRとかのときだけ使われます。

![RT-primers](https://www.thermofisher.com/jp/ja/home/life-science/cloning/cloning-learning-center/invitrogen-school-of-molecular-biology/rt-education/reverse-transcription-setup/jcr:content/MainParsys/image_aa57/foregroundimg.img.320.low.png/1500204712626.png)
...
cDNAを合成する際に、3'末端に意図的にpolyC配列を付加し、それと相補的になるようなpolyG配列を3'末端側にもつDNA oligo (template swithching oligo) を用いることで3'末端を修飾しつつSecond strand synthesisを行うことができます。この反応は、ライブラリ作成の際に3'末端側にバーコードやアダプターをつけるのに利用されます。こういった酵素は高濃度のマグネシウムやマグネシウムイオン存在下で、cDNA 合成中、あるいは反応後半に誘導されます。
...
Uracil DNA GlycosylaseとEndonuclease Ⅷの両方の活性を持つ酵素です。二本鎖DNAの中のUを除去し、nickを入れることができます。

![USER](https://international.neb.com/-/media/nebus/page-images/products/cloning-strains/user_mechanism_0118.png?rev=8746dafdeb3d4d32bb8bb3846ab372b4&hash=CFBC863E2CCBBCCF5A99E09D145FD1FF)
...
cDNAは一本鎖DNAなわけですが、基本的には二本鎖DNAのほうが扱いやすいです。なので、二本鎖DNAにするための方法があります。近年の方法はRNaseH + DNA polymerase Iを使うことが多いです。RNaseHはRNAにnickを入れる反応でDNA polymerase Iは二本鎖特異的5’→3’ exonuclease活性および一本鎖特異的3’→5’ exonuclease活性を有するDNA合成酵素です。
...
そして、切断された部分の両端にモザイク配列+アダプター配列を付加することができます。そのアダプター配列に相補的な配列を持つプライマーを用いて増幅することでライブラリを作成することができます。
